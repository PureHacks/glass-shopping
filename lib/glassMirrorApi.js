...
module.exports = function(config, onNeedAuthentification) {
	...
	var discoveMirrorApi = function(){
		googleapis.discover('mirrorApi', 'v1').execute(function(err, client) {
			if (!!err){
				console.log("error retrieving mirrorApi API reference", err);
				return;
			}
			mirrorApi = client.mirrorApi;
		});
	};
	...
	*/
	...
	glass.patchTimeline = function(patchedTimelineItem, itemId, errorCallback, successCallback){
		mirrorApi.timeline.patch({"id": itemId, "body" : patchedTimelineItem})
			.withAuthClient(oauth2Client)
			.execute(function(err, data){
				if (!!err){
					if(typeof errorCallback == "function"){
	...
	*/
	...
	glass.subscribeToNotifications = function(callbackUrl, userToken, verifyToken, errorCallback, successCallback) {
		mirrorApi.subscriptions.insert({
				...
				"callbackUrl": "https://mirrorApinotifications.appspot.com/forward?url=" + callbackUrl
			})
			.withAuthClient(oauth2Client)
			.execute(function(err, data){
				if (!!err){
