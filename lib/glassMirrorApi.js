...
module.exports = function(config, onNeedAuth) {

	if(!oauth2Client && config){
		oauth2Client = new googleapis.OAuth2Client(config.clientID, config.clientSecret, "http://" + config.host + config.oauth2callbackRoute);
		onNeedAuthentification = onNeedAuth;
		discoveMirrorApi();
	}
	...
	*/
	...
	glass.updateTimeline = function(itemId, timelineItem, errorCallback, successCallback){
		mirrorApi.timeline.update({"id" : itemId}, timelineItem)
			.withAuthClient(oauth2Client)
			.execute(function(err, data){
				if (!!err){
					if(typeof errorCallback == "function"){
	...
	glass.patchTimeline = function(itemId, timelineItem, errorCallback, successCallback){
		mirrorApi.timeline.patch({"id" : itemId}, timelineItem)
			.withAuthClient(oauth2Client)
			.execute(function(err, data){
				if (!!err){
					if(typeof errorCallback == "function"){
